...
export const api = {
  ...
  getGallery: () => request("/marketplace/gallery"),
  ...
  getAdminReports: (params, token) => request(`/admin/reports${toQueryString(params)}`, { token }),
  updateAdminReportStatus: (reportId, payload, token) => request(`/admin/reports/${reportId}`, { method: "PATCH", body: payload, token }),
  ...
  getVendorReports: (token) => request("/api/vendor/reports", { token }),
  createVendorReport: (payload, token) => request("/api/vendor/reports", { method: "POST", body: payload, token }),
